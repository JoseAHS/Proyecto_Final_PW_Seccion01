...
if (isset($_POST['form1'])) {
    ...
    elseif(filter_var($_POST['cust_email'], FILTER_VALIDATE_EMAIL) === false) { $valid = 0; $error_message .= LANG_VALUE_134."<br>"; }
    else {
    ...
    if($valid == 1) {
        ...
        // INSERT corregido (sin campos extra ni AUTO_INCREMENT)
        $statement = $pdo->prepare("INSERT INTO tbl_customer (
            cust_name, cust_cname, cust_email, cust_phone, cust_country, cust_address,
            cust_city, cust_state, cust_zip, cust_password, cust_token,
        ...
        try {
            $mail->isSMTP();
            $mail->Host       = 'smtp://live.smtp.mailtrap.io:587';
            $mail->SMTPAuth   = true;
            $mail->Username   = 'apismtp@mailtrap.io';       // Cambia por tu correo
            $mail->Password   = '';          // Contraseña de aplicación Gmail
            $mail->SMTPSecure = 'tls';
            $mail->Port       = 587;
            ...
            $mail->send();
            $success_message = LANG_VALUE_152.'<br>Correo de verificación enviado.';
        } catch (Exception $e) {
            $success_message = LANG_VALUE_152;
            $error_message .= "No se pudo enviar el correo: {$mail->ErrorInfo}";
        }
